...
public struct ContainerHostConfig: Codable {
	...
	/// forms:
	/// - `host-src:container-dest[:options]` to bind-mount a host path into the container. Both
	/// host-src, and container-dest must be an absolute path.
	/// - `volume-name:container-dest[:options]` to bind-mount a volume managed by a volume driver into
	...
	/// If not specified, the daemon default is used, which can either be "private" or "host",
	...
	/// If the path is not absolute, the path is considered to be relative to the cgroups path of the
	/// init process.
	...
	/// On Windows Server containers, the processor resource controls are mutually exclusive. The order
	/// of precedence is `CPUCount` first, then `CPUShares`, and `CPUPercent` last.
	...
	/// container:<name|id>. Any other value is taken as a custom network's name to which this
	...
	/// Maps container exposed ports to ports on the host. This is the API equivalent of the docker CLI
	/// `--publish` option.
	...
	/// Ports are de-allocated when the container stops and allocated when the container starts. The
	/// allocated port might be changed when restarting the container.
	/// The port is selected from the ephemeral port range that depends on the kernel. For example, on
	/// Linux the range is defined by `/proc/sys/net/ipv4/ip_local_port_range`.
	public var publishAllPorts: Bool = false

	/// The list of paths to be set as read-only inside the container (this overrides the default set
	/// of paths).
	...
	/// A map of container directories which should be replaced by tmpfs mounts, and their
	...
	public struct ContainerMount: Codable {
		...
		public static func bind(
			...
			to target: String,
			readOnly: Bool? = false,
			consistency: MountConsistency? = .default,
			bindOptions: BindOptions? = .init()
		) -> ContainerMount {
			ContainerMount(
				type: .bind,
				source: source,
		...
		public static func tmpFs(options: TmpfsOptions? = nil) -> ContainerMount {
			ContainerMount(type: .tmpfs, tmpFsOptions: options)
		}

		/// Consistency option for bind-mounts.
		public enum MountConsistency: String, Codable {
			/// The host's view of the mount is authoritative. There may be delays before updates made on the
			/// host are visible within a container.
			case cached

			/// Full consistency. The container runtime and the host maintain an identical view of the mount
			/// at all times.
			case consistent
			...
			/// The container runtime's view of the mount is authoritative. There may be delays before
			/// updates made in a container are visible on the host.
			case delegated
		}
		...
		public struct BindOptions: Codable {
			...
			public init(
				propagation: ContainerHostConfig.ContainerMount.BindOptions.BindOptionsPropagation = .rprivate,
			...
			public init(from decoder: Decoder) throws {
				...
				self.propagation = try container.decodeIfPresent(
					BindOptionsPropagation.self,
					forKey: .propagation
				) ?? .rprivate
				self.nonRecursive = try container.decodeIfPresent(Bool.self, forKey: .nonRecursive) ?? false
			}

			public func encode(to encoder: Encoder) throws {
				var container = encoder.container(keyedBy: CodingKeys.self)
				try container.encode(self.propagation, forKey: .propagation)
				try container.encode(self.nonRecursive, forKey: .nonRecursive)
			}

			public enum BindOptionsPropagation: String, Codable {
				/// The mount is private.
				/// Sub-mounts within it are not exposed to replica mounts, and sub-mounts of replica mounts are
				/// not exposed to the original mount.
				case `private`
				...
				/// If the original mount exposes a sub-mount, the replica mount can see it. However, if the
				/// replica mount exposes a sub-mount, the original mount cannot see it.
				case slave

				/// The same as `slave`, but the propagation also extends to and from mount points nested within
				/// any of the original or replica mount points.
				case rslave
			}
		}
	...
	public struct PortBinding: Codable {
		...
		public static func publishTo(hostIp: String, hostPort: UInt16) -> PortBinding {
			PortBinding(hostIp: hostIp, hostPort: hostPort)
		}
		...
		public init(from decoder: Decoder) throws {
			...
			guard let port = UInt16(portString ?? "") else {
				throw DecodingError.typeMismatch(
					UInt16.self,
					.init(
						codingPath: [CodingKeys.hostPort],
						debugDescription: "Must be parseable as a UInt16 value",
						underlyingError: nil
					)
				)
			}
